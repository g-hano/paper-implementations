...
def timestep_embedding(tensor_: torch.Tensor,
    ...
    #? tensor_.shape = [10]
    #? tensor_[:, None].shape = [10, 1]
    #? tensor_[None, :].shape = [1, 10]
    
    #? freqs.shape = [10]
    #? freqs[None].shape = [1, 10]

    #? args = [10, 1] * [1, 10]
    #? args.shape = [10, 10]
    args = tensor_[:, None].float() * freqs[None]

    #* [10, 10] + [10, 10] on last dim => [10, 20]
...
class RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        hidden_states = hidden_states.float() # need float to calculate mean
        
        # square tensor to make them positive
        # calculate mean on last dim.
        mean = torch.mean(hidden_states**2, dim=-1, keepdim=True)
        
        rrms = torch.rsqrt(mean + 1e-6) # 1.0 / (mean + 1e-6)

        # normalize hidden_states and multiply by learnable parameter scale
        hidden_states = (hidden_states*rrms).to(dtype=org_dtype) * self.scale
...
class SelfAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, pos_enc: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states = attention(q, k, v, pos_enc) # apply_rope changes the shape
...
class Modulation(nn.Module):
    ...
    def forward(self, vec: torch.Tensor) -> Tuple[ModulationOut, ModulationOut | None]:
        ...
        # [X, 1, Y, Z]
        out = out[:, None, :]
        out = out.chunk(self.multiplier, dim=-1) # len of self.multiplier chunks 
...
class SingleStreamBlock(nn.Module):
    ...
    def forward(self, 
        ...
        x_mod = (1 + mod.scale) * self.pre_norm(hidden_states) + mod.shift
        ...
        hidden_states = hidden_states + mod.gate * output
        return hidden_states

import math
from typing import Callable
import torch
import torch.nn as nn
from einops import rearrange, repeat
from model import Flux
from conditioner import HFEmbedder

def get_noise(
    num_samples: int,
    height: int,
    width: int,
    device: torch.device,
    dtype: torch.dtype,
    seed: int | None
):
    return torch.randn(
        num_samples,
        16,
        2 * math.ceil(height / 16),
        2 * math.ceil(width / 16),
        device=device,
        dtype=dtype,
        generator=torch.Generator(device=device).manual_seed(seed)
    )

def prepare(
        t5: HFEmbedder,
        clip: HFEmbedder,
        img: torch.Tensor,
        prompt: str | list[str]) -> dict[torch.Tensor, torch.Tensor]:
    batch_size, channel, height, width = img.shape
    # if batch_size is 1 or prompt is list
    if batch_size == 1 and not isinstance(prompt, str):
        batch_size = len(prompt)
    
    img = rearrange(img,
                    "b c (h ph) (w pw) -> b (h w) (c ph pw)",
                    ph=2, pw=2)
    if img.shape[0] == 1 and batch_size > 1:
        img = repeat(img, "1 ... -> bs ...", bs=batch_size)
    
    img_ids = torch.zeros(height//2, width//2, 3)
    img_ids[..., 1] = img_ids[..., 1] + torch.arange(height//2)[:, None]
    img_ids[..., 2] = img_ids[..., 2] + torch.arange(width//2)[None, :]
    img_ids = repeat(img_ids, "h w c -> b (h w) c", b=batch_size)

    if isinstance(prompt, str):
        prompt = [prompt]
    txt = t5(prompt)
    
    if txt.shape[0] == 1 and batch_size > 1:
        txt = repeat(txt, "1 ... -> bs ...", bs=batch_size)
    txt_ids = torch.zeros(batch_size, txt.shape[1], 3)

    vec = clip(prompt)
    if vec.shape[0] == 1 and batch_size > 1:
        vec = repeat(vec, "1 ... -> bs ...", bs=batch_size)
    
    return {
        "img": img,
        "img_ids": img_ids.to(img.device),
        "txt": txt.to(img.device),
        "txt_ids": txt_ids.to(img.device),
        "vec": vec.to(img.device),
    }

def time_shift(mu: float, sigma: float, t: torch.Tensor):
    return math.exp(mu) / (math.exp(mu) + (1 / t - 1) ** sigma)

def get_lin_function(
        x1: float = 256, 
        y1: float = 0.5, 
        x2: float = 4096, 
        y2: float = 1.15) -> Callable[[float], float]:
    m = (y2 - y1) / (x2 - x1)
    b = y1 - m * x1
    return lambda x: m * x + b

def get_schedule(
    num_steps: int,
    image_seq_len: int,
    base_shift: float = 0.5,
    max_shift: float = 1.15,
    shift: bool = True
) -> list[float]:
    timesteps = torch.linspace(1, 0, num_steps+1)

    # Shifting the schedule to favor high timesteps for higher signal images
    if shift:
        # Estimate mu based on linear estimation between two points
        mu = get_lin_function(y1=base_shift, y2=max_shift)(image_seq_len)
        timesteps = time_shift(mu, 1.0, timesteps)
    
    return timesteps.tolist()

def denoise(
    model: Flux,
    # Inputs
    img: torch.Tensor,
    img_ids: torch.Tensor,
    txt: torch.Tensor,
    txt_ids: torch.Tensor,
    vec: torch.Tensor,
    # Sampling Parameters
    timesteps: list[float],
    guidance: float = 4.0
):
    # Schnell does not uses
    guidance_vec = torch.full((img.shape[0], ), guidance,
                              device=img.device, dtype=img.dtype)
    for t_curr, t_prev in zip(timesteps[:-1], timesteps[1:]):
        t_vec = torch.full((img.shape[0], ), t_curr, dtype=img.dtype, device=img.device)
        pred = model(
            img=img,
            img_ids=img_ids,
            txt=txt,
            txt_ids=txt_ids,
            y=vec,
            timesteps=t_vec,
            guidance=guidance_vec
        )
        img += (t_prev, t_curr) * pred
    return img

def unpack(noisy_image: torch.Tensor, height: int, width: int) -> torch.Tensor:
    return rearrange(
        noisy_image,
        "b (h w) (c ph pw) -> b c (h ph) (w pw)",
        h=math.ceil(height/16),
        w=math.ceil(width/16),
        ph=2, pw=2
    )